...
package body Level_Crossing is
   ...
   begin
      ...
      -- Set system to safe initial state:
      -- - All trains at maximum distance (far away)
      -- - No trains present
      -- - Barriers up (normal traffic flow)
      -- - No sensor faults
   ...
   begin
      ...
      -- Check if train is present/being tracked
      ...
      if Status_System.Trains(Train_Number).Is_Present then
         ...
         -- Calculate and display danger distance for this train
         declare
         ...
         -- Train not present, set to safe defaults
   ...
   -- =========================================================================
   -- SAFETY-CRITICAL MONITORING LOGIC
   -- =========================================================================

   procedure Monitor_Level_Crossing is
      Must_Close_Barrier : Boolean := False;
   begin
      -- SAFETY RULE 1: If sensor fault detected, barriers MUST be down (fail-safe)
      if Status_System.Sensor_Fault then
         Must_Close_Barrier := True;
      end if;

      -- SAFETY RULE 2: Check each train against its speed-dependent danger distance
      for T in Train_ID loop
      ...
      -- Apply barrier control decision
      if Must_Close_Barrier then
         Status_System.Barrier_State := Barrier_Down;
      else
         -- Safe to raise barriers: no faults and no trains in danger zones
         Status_System.Barrier_State := Barrier_Up;
      end if;
