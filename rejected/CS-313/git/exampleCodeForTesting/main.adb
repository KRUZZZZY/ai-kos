...
procedure Main is
   ...
   -- Y will store the input by the user.
   
   Z : Natural;
   -- will be the length of the string input.
   -- so the actual input will be the first Z characters of Y
   -- which in SPARK Ada is written as
   -- Y ( 1 .. Z);
   --
   -- In general for a string Y, Y (i .. j) is the substring starting at the ith character
